...
这是一个基于 genshin-ts-touyu 的千星奇域项目模板。你可以用 TypeScript 写逻辑，编译为节点图并注入到地图。
...
- 若 VSCode/Cursor 里仍看到 `TS2538` 错误，请配置 `"typescript.tsdk": "node_modules/typescript/lib"` 与 `"typescript.enablePromptUseWorkspaceTsdk": true`（genshin-ts-touyu 的项目模板已经自带这些设置），并设置“使用工作区 TypeScript 版本”。
...
当类型提示不足时，可以直接在 `node_modules/genshin-ts-touyu` 中搜索函数/事件注释：

- 节点函数与事件定义：`node_modules/genshin-ts-touyu/dist/src/definitions/`
