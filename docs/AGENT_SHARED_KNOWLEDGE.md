...
## 10. 菜单系统架构（2026-05-28）

### 10.1 设计原则

**事件转发解耦**：菜单的事件链通过 `f.转发事件` 串联，每个实体只处理自己的职责。
...
| 图 | ID | 挂载 | 职责 |
|----|-----|------|------|
| Menu_按键转发 | 1073742448 | 角色实体 | 过滤菜单按钮 → `f.转发事件(元件7)` |
| Menu_交互菜单 | 1073742447 | 元件7 | 导航+渲染到 stage，结果写 self._menuConfirm/Result |
| Menu_命令桥接 | 1073742449 | 元件7 | 监听 _menuConfirm → switch 解码 → 写 _debugFlash 到球 |
| Ball_Debug视觉 | 1073742444 | 球 | 13 个 debug 命令执行 |

### 10.2 换菜单功能的步骤

1. 改 `menu_system.ts`：`COL1`、`SUB_0~3` 文字 + `gstsServerCol1Text`/`gstsServerSubText` 文字
2. 改 `menu_bridge.ts`：`switch(result)` 的分派逻辑
3. 渲染目标不变（stage._menu_fld1~4）

### 10.3 gstsServer 前缀的关键发现

**问题**：handler 内调用普通辅助函数（运行时参数 + 非 void 返回值），编译器静默丢弃返回值，替换为零值。

**解决**：给函数加 `gstsServer` 前缀。编译器识别后会正确内联函数调用，生成完整的 double_branch + set_local_variable 节点链。

**限制**：
- `bigint` 返回值 ✅ — 用三元链式 return
- `string` 返回值 ✅ — 同上
- `string[]` 返回值 ❌ — 游戏引擎不支持返回数组，必须内联或通过变量输出
- `switch/case` 不可用 — gstsServer 只支持单一 return 表达式，用嵌套三元替代

### 10.4 二级菜单渲染技巧

用两个 str_list 实现「前缀 + 内容」拼接：
- 字段 1/3 = 前缀（`*` 或 ` `，运行时动态切换）
- 字段 2/4 = 内容（不变或按索引换）
- 文本框并排渲染 = `*运行控制`
...
col=0（主菜单）时上下移动，必须同步更新 `mainIdx = row`，否则第二列子菜单不会跟随变化。
